...
check_os_version() {
    ...
    for version in ${LINUX_VERSION_OPTS[@]}; do
        if [ "$version" = "$LINUX_VERSION_INSTALL" ] && [ "$version" = "$HOST_OS_VERSION" ]; then
            VALID_SETUP=1
        fi
    done
    ...
    if [ $VALID_SETUP == 1 ]; then
        ...
        echo ">>>>> System setup is invalid, choose $LINUX_DISTRO version that is available (${LINUX_VERSION_OPTS[*]})."
        exit
    fi
}
