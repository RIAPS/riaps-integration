...
nethogs_prereq_install() {
    ...
    for c_arch in ${ARCHS_CROSS[@]}; do
        # for 24.04: libpcap-dev:armhf fails to install
        #sudo apt-get install libpcap-dev:$c_arch libncurses-dev:$c_arch -y
        sudo apt-get install libncurses-dev:$c_arch -y
    done
    echo ">>>>> installed nethogs prerequisites"
}

# Set apt sources list grab the released packages with draft APIs
# MM TODO: the xUbuntu_24.04 release-draft directory does not exist yet - installed from git-draft
#    --- this did not update - did not pass security of apt
zmq_draft_apt_install() {
    ...
    sudo echo "deb [signed-by=/usr/share/keyrings/zeromq-archive-keyring.gpg] http://download.opensuse.org/repositories/network:/messaging:/zeromq:/release-draft/xUbuntu_24.04/ ./" >> /etc/apt/sources.list.d/zeromq.list
    sudo apt-get update
...
zyre_czmq_prereq_install() {
    ...
    sudo apt-get install pkg-config libcurl4-gnutls-dev libnss3-dev -y
...
gnutls_install(){
    ...
    for c_arch in ${ARCHS_CROSS[@]}; do
        sudo apt-get install libgnutls30t64:$c_arch -y
    done
...
opendht_prereqs_install() {
    sudo apt-get install libncurses-dev libreadline-dev -y
    sudo apt-get install nettle-dev libasio-dev libargon2-dev -y
    sudo apt-get install libhttp-parser-dev libjsoncpp-dev -y
    sudo apt-get install libssl-dev -y
    for c_arch in ${ARCHS_CROSS[@]}; do
        sudo apt-get install libncurses-dev:$c_arch -y
        sudo apt-get install nettle-dev:$c_arch libargon2-dev:$c_arch -y
