...
opendht_prereqs_install() {
    ...
    PREVIOUS_PWD=$PWD
    chmod +x /home/$INSTALL_USER/riaps-integration/riaps-x86runtime/liblinks.sh
    for arch_tool in ${CROSS_TOOLCHAIN_LOC[@]}; do
        cd /usr/lib/$arch_tool
        sudo /home/$INSTALL_USER/riaps-integration/riaps-x86runtime/liblinks.sh
    done
    cd $PREVIOUS_PWD
...
riaps_prereq() {
    ...
    sudo apt-get update
    sudo cp /home/$INSTALL_USER/riaps-integration/riaps-x86runtime/riaps_install_vm.sh /home/$RIAPSUSER/.
